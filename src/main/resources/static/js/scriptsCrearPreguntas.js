	...
	function agregarOpcionCorrelacion() {
		...
		var texto = '<input type="text" id="'  + id + '" onchange ="cargarCeldaOpcionCorrecta(this)"/>';
	
		var ingresoValor = '<td > <div> ' + texto + ' </div> </td>';
		
		var valorIngresado = '"<td id="td' + id+ '"></td>"';

		var botonBorrar = '<td><a class="btn btn-warning btn-sm" id="botonBorrar" onclick="quitar(this)">Quitar</a></td>'

		var fila = '<tr>' + colEnunciado + ingresoValor + valorIngresado + botonBorrar + '</tr>';

		//agrega fila a la tabla
		$('#tabla-opciones tr:last').after(fila);
	}
	
	function cargarCeldaOpcionCorrecta(cuadroTexto) {
	...
	//recorro la tabla y obtengo el valor del radio button que esta seleccionado
